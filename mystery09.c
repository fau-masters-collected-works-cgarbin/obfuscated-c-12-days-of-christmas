...
f(t, _, a) char *a;
{
    if (t == GOTO_TO_SLASH) {
        ...
        int tmp = f(ADVANCE_OR_PRINT, *a, small_string);
        return f(GOTO_TO_SLASH, tmp, a + 1);
    }
    ...
    if (t == 2) {
        int tmp1 = f(SWAP_VARIABLES, 0, a + 1);
        int tmp2 = f(SWAP_VARIABLES, 1 - _, tmp1 + a);
        f(SWAP_VARIABLES, -13, a + tmp2);
    }

    if (t < (SWAP_VARIABLES+1)) {
        return f(_, t, large_string);
    }

    if (t < (ADVANCE_OR_PRINT+1)) {
        if (_ == *a) {
            return putchar(a[31]);
        }
        return f(ADVANCE_OR_PRINT, _, a + 1);
    }
    ...
    if (t > 1) {
        ...
        if (f(SWAP_VARIABLES, -27 + t, a) && t == 2) {
            if (_ < 13) {
